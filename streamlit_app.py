...
# --- Tab 9: About / דו"ח מסכם (מעודכן) ---
with tab_about:
    st.header("ℹ️ About / דו\"ח מסכם")

    with st.expander("🎯 פתיחה – רקע ומטרה"):
        st.markdown("""
        מחלת פרקינסון היא מחלה נוירולוגית מתקדמת, שמתבטאת בעיקר בהפרעות תנועה ורעידות.  
        סימנים מוקדמים מופיעים בקול – רעידות בתדר, חוסר יציבות בעוצמה ושינויים עדינים.  

        המטרה שלנו: לבדוק האם אפשר **לחזות פרקינסון על סמך קול בלבד** בעזרת למידת מכונה.  

        **למה זה חשוב?**  
        - כלי לא פולשני (רק הקלטת קול).  
        - כלי זול ונגיש.  
        - מאפשר גילוי מוקדם וטיפול מהיר.  
        """)

    with st.expander("🔑 מה זה פיצ’ר?"):
        st.markdown("""
        פיצ’ר (Feature) = מאפיין כמותי של הקול (עמודה בטבלה).  

        **דוגמאות מרכזיות:**  
        - **Fo:** תדר בסיסי של הקול.  
        - **Fhi, Flo:** תדר מקסימלי ומינימלי.  
        - **Jitter:** רעידות בתדר (גבוה יותר אצל חולים).  
        - **Shimmer:** חוסר יציבות בעוצמה (גבוה יותר אצל חולים).  
        - **NHR:** יחס רעש־הרמוניות (גבוה יותר אצל חולים).  
        - **Spread1 / Spread2, PPE:** מדדים מחושבים בעלי מתאם חזק מאוד למחלה.  

        👉 אפשר לחשוב על פיצ’רים כ"מדדים פיזיולוגיים" של הקול שבעזרתם המודל יודע להבדיל בין חולים לבריאים.
        """)

    with st.expander("📊 שלב ראשון – ניתוח מקדים (EDA)"):
        st.markdown("""
        **למה עושים EDA?** כדי להכיר את הנתונים, למצוא דפוסים ולוודא שניתן לחזות את המטרה.  

        **ממצאים עיקריים:**  
        - **Jitter:** חולים ≈ 0.0071, בריאים ≈ 0.0035.  
        - **Shimmer:** חולים ≈ 0.035, בריאים ≈ 0.022.  
        - **Heatmap:** קשר חזק בין jitter ל־shimmer → מעיד על בעיה במערכת הקול.  
        - **PCA:** חולים ובריאים נפרדו במרחב דו־ממדי → יש סיגנל חזק.  
        - **t-SNE:** אשכולות ברורים → חולים מתרכזים באזור מוגדר.  
        """)

    with st.expander("🎯 שלב שני – בחירת פיצ’רים"):
        st.markdown("""
        בדקנו האם כדאי להשתמש בכל הפיצ’רים או לבחור את החשובים ביותר.  

        **שיטות:**  
        - SelectKBest – דירוג לפי תרומה ליעד.  
        - RFE – הורדה הדרגתית של פיצ’רים פחות חשובים.  
        - VIF – בדיקת מתאם בין פיצ’רים.  

        **מסקנה:**  
        - **פיצ’רים מחושבים (Spread1, PPE, Spread2)** → הכי מתואמים סטטיסטית למחלה.  
        - **פיצ’רים בסיסיים (Jitter, Shimmer)** → הכי אינטואיטיביים להסבר.  
        - בפרויקט שילבנו את שניהם → מודל גם חזק וגם מובן.  
        """)
    ...
    with st.expander("🤖 שלב שלישי – בניית מודלים"):
        ...
        1. **NeuralNet (MLP)** – ROC-AUC ≈ 0.983 → הטוב ביותר סטטיסטית.  
        2. **CatBoost** – ROC-AUC ≈ 0.983 → תוצאה זהה ל־NeuralNet.  
        3. **XGBoost** – ROC-AUC ≈ 0.976 → מעט נמוך יותר, אבל הרבה יותר מוסבר ויציב.  
        4. **Random Forest, KNN, SVM** – ROC-AUC סביב 0.95–0.96.  
        5. **Logistic Regression** – ROC-AUC ≈ 0.92 → בסיס להשוואה.  

        👉 **החלטה סופית:** למרות ש־NeuralNet ו־CatBoost מובילים מספרית, נבחר ב־**XGBoost** כמודל המרכזי בזכות יציבות ו־Explainability.
        """)

    with st.expander("⚙️ שלב רביעי – Grid Search"):
        st.markdown("""
        Grid Search = הרצה חוזרת עם שילובי פרמטרים שונים כדי למצוא את הטוב ביותר.  

        **דוגמאות לשיפורים:**  
        - **Random Forest:** Recall עלה ל־91% עם 200 עצים.  
        - **XGBoost:** ROC-AUC עלה ל־0.976 עם max_depth=6 ו־learning_rate=0.1.  
        - **NeuralNet:** יציבות טובה יותר והפחתת Overfitting.  
        """)

    with st.expander("📈 שלב חמישי – הערכת ביצועים"):
        st.markdown("""
        בדקנו ביצועים במספר דרכים:  

        - **Confusion Matrix (XGBoost, 60 דגימות):** TP=27, TN=27, FP=3, FN=3.  
        - **ROC Curve:** NeuralNet ו־CatBoost מובילים, XGBoost קרוב מאוד.  
        - **Precision-Recall Curve:** XGBoost שמר על איזון מצוין.  
        - **Learning Curves:** XGBoost יציב, NeuralNet נטה ל־Overfitting.  
        """)

    with st.expander("🧠 שלב שישי – Explainability"):
        st.markdown("""
        ברפואה אסור להסתפק בתחזית → חייבים להבין למה.  

        **מה גילינו:**  
        - **Jitter & Shimmer** – הכי חשובים להסבר.  
        - **Spread1, PPE, Spread2** – נתנו עוצמה סטטיסטית.  
        - SHAP הראה: jitter גבוה → חולה, jitter נמוך → בריא.  

        👉 ההסברים תאמו ידע רפואי קיים → העלה אמון במודל.  
        """)

    with st.expander("✅ מסקנות סופיות"):
        st.markdown("""
        1. NeuralNet ו־CatBoost מובילים מספרית (ROC-AUC ≈ 0.983).  
        2. XGBoost נבחר כמוביל בזכות יציבות ויכולת הסבר (SHAP).  
        3. הפיצ’רים הקריטיים ביותר: Jitter, Shimmer, Spread1, PPE.  
        4. Grid Search תרם לשיפור משמעותי.  
        5. המערכת נותנת גם חיזוי מדויק וגם שקיפות – חיוני לרפואה.  
        """)

    with st.expander("✨ נקודות מעניינות"):
        st.markdown("""
        - גם Logistic Regression פשוט הראה דפוסים → סימן שהקול אינדיקטור חזק.  
        - Random Forest כמעט ולא פספס חולים, אבל יצר יותר אזעקות שווא.  
        - NeuralNet היה החזק ביותר מספרית, אבל קשה להסבר.  
        - XGBoost היה הבחירה המאוזנת ביותר.  
        """)
...
with tab_qa:
    ...
    with st.expander("📊 מאיפה הגיעו הנתונים ואיך נאספו?"):
        ...
        הם נאספו מהקלטות קול של חולי פרקינסון ושל בריאים.  
        מכל הקלטה הופקו עשרות פיצ’רים מתמטיים (jitter, shimmer, PPE).  
        """)

    with st.expander("📊 למה דווקא קול ולא נתונים אחרים?"):
        st.markdown("""
        קול הוא כלי **לא פולשני**, **זול** ו־**נגיש**.  
        מספיקת הקלטה קצרה כדי להפיק נתונים לחיזוי.  
        """)

    with st.expander("📊 האם הדאטה היה מאוזן?"):
        st.markdown("""
        לא. היו יותר חולים מבריאים → חוסר איזון (class imbalance).  
        לכן הסתמכנו גם על Recall, Precision ו־F1 ולא רק על Accuracy.  
        """)

    # --- על הפיצ’רים ---
    with st.expander("🔑 מה זה פיצ’ר?"):
        st.markdown("""
        פיצ’ר = מאפיין מספרי של הקול.  
        - **Jitter:** רעידות בתדר.  
        - **Shimmer:** יציבות בעוצמה.  
        - **Fo:** תדר בסיסי.  
        - **Spread1, PPE, Spread2:** מדדים מחושבים עם מתאם גבוה למחלה.  
        """)

    with st.expander("🔑 אילו פיצ’רים יצאו החשובים ביותר?"):
        st.markdown("""
        - **NeuralNet / CatBoost (Correlation):** Spread1, PPE, Spread2 → הכי חזקים סטטיסטית.  
        - **Explainability (SHAP):** Jitter ו־Shimmer → הכי חשובים להסבר.  
        """)

    # --- על המודלים ---
    with st.expander("🤖 מה המודל המוביל בפרויקט?"):
        st.markdown("""
        סטטיסטית, **NeuralNet ו־CatBoost** הובילו (ROC-AUC ≈ 0.983).  
        בפועל, נבחר ב־**XGBoost** בזכות יציבות ויכולת הסבר (Explainability).  
        """)

    with st.expander("🤖 למה לא נבחר NeuralNet או CatBoost?"):
        st.markdown("""
        - NeuralNet → Black Box, קשה להסבר לרופאים.  
        - CatBoost → תוצאות מצוינות, אבל פחות אינטואיטיבי להסבר רפואי.  
        - XGBoost → תוצאה כמעט זהה, אבל הרבה יותר יציב ובר הסבר.  
        """)

    with st.expander("🤖 יתרונות וחסרונות XGBoost"):
        st.markdown("""
        ✅ דיוק גבוה (ROC-AUC ≈ 0.976).  
        ✅ יציב על דאטה קטן.  
        ✅ Explainability עם SHAP.  
        ❌ דורש יותר חישוב.  
        ❌ מעט פחות מדויק מה־NeuralNet.  
        """)

    # --- על Grid Search ---
    with st.expander("⚙️ מה תרם Grid Search?"):
        st.markdown("""
        Grid Search שיפר Recall ו־ROC-AUC על ידי כוונון פרמטרים.  
        לדוגמה: XGBoost השתפר ל־ROC-AUC ≈ 0.976 עם max_depth=6.  
        """)

    # --- על ההשלכות ---
    with st.expander("📈 מה המדד הכי חשוב בפרויקט?"):
        st.markdown("""
        **Recall** – עד כמה המודל מזהה חולים אמיתיים.  
        ברפואה חשוב לא לפספס חולים.  
        """)

    with st.expander("📈 מה המשמעות של ROC-AUC = 0.983 אצל NeuralNet?"):
        st.markdown("""
        זה אומר שהמודל מבחין בין חולים לבריאים בכ־98% מהמקרים ללא קשר לסף מסוים.  
        """)

    with st.expander("🏥 האם המודל הזה מוכן לשימוש רפואי?"):
        st.markdown("""
        עדיין לא. זה כלי מחקרי בלבד.  
        נדרשים מחקרים קליניים נרחבים ואישור רגולטורי.  
        """)

    with st.expander("🏥 מה השלב הבא?"):
        st.markdown("""
        - לאסוף דאטה גדול ומגוון יותר.  
        - לבדוק רשתות עמוקות יותר.  
        - לבצע מחקר קליני לבדיקת המודל בעולם האמיתי.  
        """)
